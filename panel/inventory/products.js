  ...
  function initProducts() {
    ...
    Promise.all([
      ...
      .then(([categories, brands]) => {
        ...
        brandMap = brands.reduce((map, brand) => {
                ...
                fetch(`http://127.0.0.1:3000/api/products/${product.product_id}/quantity-on-hand`)
                  ...
                  .then(quantityOnHand => {
                    ...
                    addNewProductRow(
                      ...
                      product.product_id // Pass product ID for deletion
                    );
                  });
              });
      ...
      addProductForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          addNewProductRow(
            ...
            categoryMap[newProduct.category_code] || newProduct.category_code, // Get category name from map
            brandMap[newProduct.brand_id] || newProduct.brand_id, // Get brand name from map
            newProduct.price,
            'In Stock', // Default status
            newProduct.product_id // Pass product ID for deletion
          );
    ...
    function addNewProductRow(imgSrc, name, category, brand, price, status, quantityInStock = 0, productId) {
      ...
      parentRow.innerHTML = `
        ...
        <td>
          ...
          <i class="fa-solid fa-trash delete-product" data-product-id="${productId}"></i>
        </td>
      `;
    ...
    function openEditModal(parentRow) {
      ...
      const productId = parentRow.querySelector(".delete-product").getAttribute('data-product-id');
    ...
    // 11. Handle Edit Product Form Submission
    ...
    if (editProductForm) {
      editProductForm.addEventListener("submit", async (e) => {
        ...
        const editModal = document.getElementById("editProductModal");
        const productId = editModal.getAttribute('data-product-id');
